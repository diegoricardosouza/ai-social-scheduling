...
export async function GET(request: NextRequest) {
  try {
    ...
    const [typesRes, userChannelRes] = await Promise.all([
    ...
    if(typesRes.error || userChannelRes.error) {
      return new NextResponse("Internal Server Error", { status: 500 });
    }

    const userChannelMap = new Map(
      userChannelRes.data.map(channel => [
    ...
    return NextResponse.json({ channels, totalChannels, connectedCount });
